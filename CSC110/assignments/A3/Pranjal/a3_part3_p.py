...
def part3_warmup() -> None:
    ...
    for i in range(0, 100):
        point = (int(8 * i), int(8 * i))
        a3_helpers.draw_pixel(screen, point, a3_helpers.float_to_colour(i / 99))
...
def generate_point_sequence1(vertex_points: list[tuple[int, int]],
    ...
    for i in range(1, num_points):
        ...
        sequence.append(calculate_midpoint(sequence[i - 1], random_vertex))
...
def draw_point_sequence1(
    ...
    a3_helpers.draw_pixel(screen, initial_point, a3_helpers.float_to_colour(1))
    previous_point = initial_point

    for _ in range(1, num_points):
        random_vertex = vertex_points[random.randint(0, len(vertex_points) - 1)]
        new_point = calculate_midpoint(previous_point, random_vertex)
        a3_helpers.draw_pixel(screen, new_point, a3_helpers.float_to_colour(1))
        previous_point = new_point

    a3_helpers.wait_for_pygame_exit()


def calculate_midpoint(point1: tuple[int, int], point2: tuple[int, int]) -> tuple[int, int]:
...
def generate_point_sequence2(vertex_points: list[tuple[int, int]],
    ...
    # for i in range(1, num_points):
    #     if i >= 3 and indices[0] == indices[1]:
    #         index = (indices[1] + random.randint(2, len(vertex_points) - 2)) % len(vertex_points)
    #     else:
    #         index = random.randint(0, len(vertex_points) - 1)
    #
    #     random_vertex = vertex_points[index]
    #     sequence.append(calculate_midpoint(sequence[i - 1], random_vertex))
...
def draw_point_sequence2(
    ...
    for _ in range(1, num_points):
        if _ >= 3 and indices[0] == indices[1]:
            index = (indices[1] + random.randint(2, len(vertex_points) - 2)) % len(vertex_points)
        else:
            index = random.randint(0, len(vertex_points) - 1)
...
def verify_point_sequence1(vertex_points: list[tuple[int, int]],
    ...
    for i in range(1, len(points)):
        if not any({points[i] == calculate_midpoint(points[i - 1], vertex) for vertex in vertex_points}):
            return False

    return True


def verify_point_sequence2(vertex_points: list[tuple[int, int]],
                           initial_point: tuple[int, int],
                           points: list[tuple[int, int]]):
    """Return whether the given points could have been generated as "Point Sequence 2" for the
    given vertex_points and initial_point.
    ...
    for i in range(1, len(points)):
        correct_vertex = [points[i] == calculate_midpoint(points[i - 1], vertex) for vertex in vertex_points]
        if any(correct_vertex):
            indices.append(correct_vertex.index(True))
        else:
            print(indices)
            return False
    ...
    for i in range(2, len(indices)):
        if indices[i - 1] == indices[i - 2]:
            if abs(indices[i - 1] - indices[i]) in {0, 1, len(vertex_points) - 1}:
                print(indices)
                return False
...
def user_pattern(screen_width: int, screen_height: int, num_vertices: int, sequence_type: int, num_points: int) -> None:
    ...
    for _ in range(0, num_vertices + 1):
        points.append(a3_helpers.input_mouse_pygame())

    initial_point = points[num_vertices]
    vertex_points = [points[i] for i in range(0, num_vertices)]

    if sequence_type == 1:
        draw_point_sequence1(screen_width, screen_height, vertex_points, initial_point, num_points)
