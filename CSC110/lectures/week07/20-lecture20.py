...
def rsa_encrypt(public_key: tuple[int, int], plaintext: int) -> int:
    ...
    return pow(plaintext, e, n)  # Better than: plaintext ** e % n
...
def rsa_generate_key(p: int, q: int) -> \
    ...
    # Step 3: Pick e.
    e = random.randint(2, phi_n - 1)  # Pick an initial choice
    while math.gcd(e, phi_n) > 1:
        e = random.randint(2, phi_n - 1)  # Pick another random choice
