...
def test_collision_simple() -> None:
    ...
    m2 = 'Cryptography in book'

    # Check that the two messages are distinct
    assert m1 != m2

    # Check that the two messages generate the same signature
...
def find_collision_simple(message: str) -> str:
    ...
    True
    """
    characters = [chr((ord(c) + 1) % 128) for c in message]
...
def len_times_sum(message: str) -> int:
    ...
    - message != ''
    """
    return sum([ord(c) for c in message]) * len(message)
...
def ascii_to_int(message: str) -> int:
    ...
    return a4_part3.base128_to_int(ords)
